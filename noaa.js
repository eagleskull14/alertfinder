  ...
  let warnings = JSON.parse(
    noaa(
      "https://api.weather.gov/alerts/active?area=" +
        JSON.parse(noaa("https://api.weather.gov/points/" + points, false))
...
document.getElementById("info").onsubmit = () => {
  if (cord) {
    if (
      ...
      try {
        updateText(
          pointWarns(
            parseFloat(document.getElementById("ps").value.split(",")[0]),
            parseFloat(document.getElementById("ps").value.split(",")[1])
          ).length,
          pointWarns(
            parseFloat(document.getElementById("ps").value.split(",")[0]),
            parseFloat(document.getElementById("ps").value.split(",")[1])
  ...
  if (geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      try {
        updateText(
          pointWarns(position.coords.latitude, position.coords.longitude)
            .length,
          pointWarns(position.coords.latitude, position.coords.longitude),
          JSON.parse(
            noaa(
  ...
  try {
    updateText(
      stateWarns(document.getElementById("state").value.toUpperCase()).length,
...
function updateText(warns, warnlist, state, points) {
  if (warns != 0 || warnlist == []) {
    ...
    if (points != undefined) {
      text += ", at the cords of " + points + ", there ";
    } else {
      text = text + ", there ";
    }
    text =
    ...
    for (let warn of warnlist) {
      ...
      totalhtml =
        ...
        `". It expires at ` +
        warn.properties.expires +
        ` and ends (if not null) at ` +
        warn.properties.ends +
        `
                </p>
                </details>
              </div>
  ...
  let text = "In the state of " + state;
  if (points != undefined) {
    text += ", at the cords of " + points + ", there are no weather alerts.";
  } else {
